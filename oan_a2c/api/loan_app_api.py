...
def _validate_jwt():
    # If running unit tests and no mock/real HTTP request context is set, bypass
    if frappe.flags.in_test and (not hasattr(frappe.local, "request") or not frappe.local.request or not getattr(frappe.local.request, "path", "")):
        return
    ...
    if path in [
        ...
        "/api/method/oan_a2c.api.loan_app_api.login",
        "/api/method/oan_a2c.api.loan_app_api.forgot_password",
        "/api/method/oan_a2c.api.loan_app_api.reset_password"
    ]:
        return
